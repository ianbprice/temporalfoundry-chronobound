"""
DaggerheartRules v1.0.2c
========================

SRD-compliant implementation of the Daggerheart RPG system for ChronoBound.

Changelog v1.0.2c:
1. Enhanced raw_damage_rolls to include individual die faces instead of total
2. Documented _pending_damage hint mechanism in mark_stress()
...
class DaggerheartRuleSystem:
    ...
    def make_attack_roll(
        ...
        **kwargs
        ...
        if hit:
            ...
            # Use individual die faces for raw_damage_rolls
            raw_damage_faces = normal.individual_rolls
        ...
        return AttackResult(
            ...
            raw_damage_rolls=raw_damage_faces,
    ...
    def mark_stress(self, character: Dict[str, Any], amount: int = 1) -> Dict[str, Any]:
        ...
        Optional _pending_damage hint: When use_threshold_stress=True, this method
        checks for character["_pending_damage"] with shape {"prev_hp": int, "new_hp": int}.
        If found, calculates additional stress based on HP damage thresholds and adds
        it to the requested amount. The hint is automatically removed after use (one-shot).
        """
...
if __name__ == "__main__":
    print("Running DaggerheartRules v1.0.2c self-tests...")
    ...
    # Test 4: Attack damage breakdown with raw rolls
    print("\n4. Testing attack damage breakdown with raw die faces...")
    ...
    # Test raw_damage_rolls contains individual die faces, not damage total
    assert len(attack_result.raw_damage_rolls) == 2, f"Expected 2 raw damage rolls, got {len(attack_result.raw_damage_rolls)}"
    assert attack_result.raw_damage_rolls == breakdown["rolls"], f"raw_damage_rolls should match breakdown rolls"
    print(f" ✓ Attack breakdown with raw faces: {attack_result.raw_damage_rolls}")
    ...
    assert info["version"] == "1.0.2c", f"Expected v1.0.2c, got {info['version']}"
    print(f" ✓ Metadata: version={info['version']}, crit_condition='{info['crit_condition']}'")
    
    print("\n✅ All self-tests passed! DaggerheartRules v1.0.2c enhancements working correctly.")
